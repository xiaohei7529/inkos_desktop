...
主题：**新建向导大纲生成 + LLM 稳定性增强**。扩展新建小说向导的 AI 辅助能力，并优化写作页上下文展示与长请求处理。

### 新增

- **新建小说向导 · AI 估算篇幅**：根据标题、题材、故事核心、角色、世界观与文风，调用写作 LLM 估算 `target_chapters` 与 `chapter_word_goal`，并解析 JSON 后填入项目设置。
- **新建小说向导 · AI 生成 outline.md**：按目标章节数生成可直接写入 `story_state/outline.md` 的书籍大纲，包含宏观大纲、分卷/阶段大纲与章节级细纲；创建小说时若尚无大纲，会先生成再落盘。
- `inkoswin_prompt` 新增 `generate_project_budget_prompt`、`parse_project_budget_output`、`generate_outline_prompt`、`parse_outline_output`，并补充对应解析测试。

### 优化

- **章节标题解析保护**：新增 `resolve_chapter_title`，在章节文件标题为空或仅为“第N章”时优先使用项目元数据标题，减少生成/预览/定时写作链路的标题回退漂移。
- **写作页上下文雷达**：默认折叠“当前激活上下文”，展开后改为可滚动文件列表，折叠态显示注入档案数量与前几个文件名，降低页面占用。
- **流式生成预览**：仅在模型标题与 fallback 不一致或当前标题为空时更新标题；正文变化时标记 dirty，避免流式预览丢失待保存状态。
- **LLM 请求稳定性**：HTTP 总超时提升到 300 秒；流式任务记录 `last_event_at`，若 90 秒没有新内容则标记中断并保留已生成文本；通道异常断开时给出明确错误。
